...
|                    |                    | | ``2i62``: 2nd-order with 6th(xy)/2nd(z)-order interpolation |
...
+-----------------------+-------------------------+--------------------------------------------+
| ``tdep_aermr{01-11}`` | ``[aerosol] swtimedep`` | Aerosol individual time depenence switches |
+-----------------------+-------------------------+--------------------------------------------+

Boundary conditions ``[boundary]`` and ``[land_surface]``
---------------------------------------------------------
...
For ``swboundary=surface_lsm``, the ``land_surface`` group contains additional settings specific for the HTESSEL land-surface model. 
...
When using ``swhomogeneous=true``, the following surface/vegetation parameters need to be specified. 
